...
class Tag extends events_1.EventEmitter {
    ...
    parseReadMessageResponseValueForAtomic(data) {
        const { SINT, INT, DINT, REAL, BOOL, LINT, BIT_STRING, UINT, SHORT_STRING, USINT } = data_types_1.Types;
        ...
        switch (this.state.tag.type) {
            ...
            case SHORT_STRING: // single byte character string
                ...
                break;
            default:
                const ttt = enip_1.CIP.DataTypes.getTypeCodeString(this.state.tag.type);
                throw new Error(`Unrecognized Type Passed Read from Controller: ${this.state.tag.type} ${ttt}`);
        }
        /* eslint-enable indent */
    }
    /**
    ...
    generateWriteMessageRequestForAtomic(value, size) {
        const { tag } = this.state;
        const { BIT_STRING, SINT, INT, DINT, REAL, BOOL, LINT, SHORT_STRING, USINT } = data_types_1.Types;
        // Build Message Router to Embed in UCMM
     ...
     * @memberof Tag
     */
    generateWriteMessageRequestFrag(offset = 0, value, size = 0x01) {
        const { tag } = this.state;
        const { BIT_STRING, SINT, INT, DINT, REAL, BOOL, LINT, SHORT_STRING } = data_types_1.Types;
        // Build Message Router to Embed in UCMM
